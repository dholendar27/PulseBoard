  ...
  async (req: Request, res: Response) => {
    ...
    if (!parsedIncidentData.success) {
      return res.status(400).json({
        status: false,
        message: z.treeifyError(parsedIncidentData.error),
      });
    }
    if (parsedIncidentData.data.assignee) {
      ...
      if (!assignee_user) {
        return res
          .json(400)
          .json({ status: false, message: "Assignee not found" });
      }
    }
    ...
    const new_incident = await prisma.incident.create({
      data: {
        ...
        assigned_to_id: parsedIncidentData.data.assignee
          ? parsedIncidentData.data.assignee
          : user.id,
      },
    });
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        assigned_to: new_incident.assigned_to_id,
        created_by: new_incident.created_by_user_id,
      },
    });
  },
);
...
incidentRouter.get("/all", async (req: Request, res: Response) => {
  ...
  if (incidents.length === 0) {
    return res
      .status(200)
      .json({ status: false, message: "No incidents found" });
  }
  return res.status(200).json({
    status: true,
    message: "Incidents retrieved successfully",
...
incidentRouter.get("/:incidentId", async (req: Request, res: Response) => {
  ...
  if (!params.incidentId) {
    return res
      .status(400)
      .json({ status: false, message: "incidentId is required" });
  }
...
incidentRouter.put(
  ...
  async (req: Request, res: Response) => {
    try {
      const user = req.user;
      const params = req.params;
      const parsedUpdatedIncidentData = incidentUpdateValidator.safeParse(
        req.body,
      );
      ...
      if (!parsedUpdatedIncidentData.success) {
        return res.status(400).json({
          status: false,
          message: z.treeifyError(parsedUpdatedIncidentData.error),
        });
      }
      ...
      if (!incident) {
        return res
          .status(404)
          .json({ status: false, message: "Incident not found" });
      }

      if (
        incident.status === "RESOLVED" &&
        incident.assigned_to_id !== parsedUpdatedIncidentData.data?.assignee
      ) {
        return res.status(200).json({
          status: true,
          message: "Assignee can't be changed once the incident is resolved",
        });
      }
...
incidentRouter.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    return res
      .status(200)
      .json({ status: true, message: "Status is changed", data: incident });
  },
);
